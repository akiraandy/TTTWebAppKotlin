...
internal class TTTEngineParserTest {
    ...
    private val computerMakeMove: GameData = GameData(arrayListOf("X", "1", "2", "3", "4", "5", "6", "7", "8"), null)
    private val computerFullBoard: GameData = GameData(arrayListOf("X", "X", "X", "X", "X", "X", "O", "O", "O"), null)
    ...
    internal fun returnsANewBoard() {
        ...
        val newBoard = tttEngineParser.generateBoard(emptyBoardHumanMove.board, emptyBoardHumanMove.input)
    ...
    internal fun computerTurnReturnsNewBoard() {
        ...
        val newBoard = tttEngineParser.generateBoard(computerMakeMove.board, computerMakeMove.input)
        assertEquals(expected, newBoard)
    }
    ...
    internal fun computerMoveReturnsSameBoardIfGameIsOver() {
        ...
        val newBoard = tttEngineParser.generateBoard(computerFullBoard.board, computerFullBoard.input)
        assertEquals(expected, newBoard)
    }
    ...
    internal fun returnsSameBoardIfHumanMakesInvalidInput() {
        ...
        val newBoard = tttEngineParser.generateBoard(invalidInput.board, invalidInput.input)
        assertEquals(expected, newBoard)
    }
}